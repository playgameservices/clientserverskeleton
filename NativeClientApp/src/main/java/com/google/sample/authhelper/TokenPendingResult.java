 ...
 * pending result to be returned to the caller, and then updated when available,
 * simplifying the handling of callbacks.
 */
public class TokenPendingResult extends PendingResult<TokenResult> {
    ...
    @Override
    public void setResultCallback(
            ResultCallback<? super TokenResult> resultCallback, long l,
            TimeUnit timeUnit) {
        try {
            if (!latch.await(l, timeUnit)) {
                setResult(null, null, null, CommonStatusCodes.TIMEOUT);
    ...
    private synchronized void setCallback(
            ResultCallback<? super TokenResult> callback) {
    ...
    /**
     * Set the result.  If any of the values are null, and a previous
     * non-null value was set, the non-null value is retained.
     *
    ...
    private synchronized void setResult(String authCode, String email, String
        ...
        String atok = (result != null && authCode == null) ?
                result.getAuthCode() : authCode;
        String itok = (result != null && idToken == null) ?
                result.getIdToken() : idToken;
        String em = (result != null && email == null) ?
